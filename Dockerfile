...
RUN apt-get update && apt-get install -y --force-yes \ 
    g++ cmake libboost-dev python-dev libglib2.0-dev libpixman-1-dev liblua5.2-dev swig libcap-dev git vim libattr1-dev wget perl-modules \
...
RUN apt-get update && apt-get install -y --force-yes openjdk-7-jre-headless libyaml-perl libcapture-tiny-perl libxml-simple-perl
...
RUN cmake -DCMAKE_INSTALL_PREFIX=/usr/local/vp -DSYSTEMC_PREFIX=/usr/local/systemc-2.3.0 -DNVDLA_HW_PREFIX=/usr/src/hw -DNVDLA_HW_PROJECT=$NVDLA_CONFIGURATION \
    && make -j 8 \
    && make install \
    && make clean \
